...
const PurposeTile = ({text, isSelected, onClick} : {text: string, isSelected: boolean, onClick: React.MouseEventHandler<HTMLDivElement>}) => {
  const getStyles = () => {
    if (isSelected) {
      return "mt-3 border-[1px] rounded bg-content-main/5 border-content-main/20 w-full py-3 px-5 shadow-sm active:bg-transparent cursor-pointer";
    }
    return "mt-3 border-[1px] rounded bg-content-main/5 border-content-main/20 w-full py-3 px-5 shadow-sm active:bg-transparent hover:shadow cursor-pointer";
  }
  return (
